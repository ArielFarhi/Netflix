...
const NewFilm = ({ setSelectedMovie, type }) => {
  const { data: newItems, isLoading, error } = usePrograms({
    query: "newest",
    type,
  });
  ...
  if (error) {
    return <div>Failed to load new content.</div>;
  }

  return (
    <div className="mt-8 w-full">
      <h3 className="font-medium text-[20px] mb-3 relative z-10">
        New {type === "tv" ? "TV Shows" : "Movies"}
      </h3>
